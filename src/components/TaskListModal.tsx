      ...
      <div className="w-72 bg-surface dark:bg-surface-dark-elevated rounded-2xl border border-outline dark:border-outline-dark shadow-[0_8px_32px_rgba(0,0,0,0.12)] overflow-hidden">
        ...
        <div className="px-3 pb-2">
          <div className="relative flex items-center">
            <Search className="absolute left-3 w-3.5 h-3.5 text-on-surface-muted dark:text-on-canvas-dark-muted" />
            <input
              ref={inputRef}
              type="text"
              value={query}
        ...
        <div className="px-2 pb-3 max-h-56 overflow-y-auto custom-scrollbar space-y-0.5">
          {filtered.length === 0 ? (
            <p className="px-3 py-4 text-center text-[12px] text-on-surface-muted dark:text-on-canvas-dark-muted">{t('tasks.noMatch')}</p>
          ) : (
            filtered.map((task) => (
              <button
                key={task.id}
