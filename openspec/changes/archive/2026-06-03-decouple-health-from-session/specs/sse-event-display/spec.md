...
Defines how the renderer displays four newly relayed SSE events — subagent lifecycle, context compaction, provider retry, and user interruption — that are currently acknowledged but not surfaced in the UI (no-op listeners at `src/session/events.ts:211-223`).
