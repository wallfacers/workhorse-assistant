...
- [x] 1.5 Implement `handleScroll` with `lastScrollTop` tracking: `movedUp && distanceFromBottom > 0` → pause; near bottom (≤ `RE_ENGAGE_PX` = 60px) → re-engage
...
- [x] 2.1 Add `shiki` (^4), `marked` (^18), `morphdom` (^2), `dompurify` (^3) to `package.json` and install
- [x] 2.2 Create `src/components/chat/highlighter.ts`: lazy memoized Shiki singleton (`github-light` + `github-dark`, the 27 langs from design D2) using the JS RegExp engine (`createJavaScriptRegexEngine()`), exporting `highlightCode(lang, code)` with `themes:{light,dark}, defaultColor:false`
- [x] 2.3 Rewrite `MarkdownContent.tsx` as a `marked` + `DOMPurify` + `morphdom` renderer (no `react-markdown`); add `src/components/chat/markdown-stream.ts` to split in-flight Markdown into stable prose blocks + an in-progress code fence (`renderStreamingCodeBlock`)
...
- [x] 4.3 Added `on-surface-muted` / `on-surface-dark-muted` muted-foreground tokens to `docs/DESIGN.md` and mirrored them into the `@theme` block in `src/index.css`; code-block chrome now uses tokens + `color-mix` washes (no hand-tuned hex)
