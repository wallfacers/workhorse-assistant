...
- [ ] 1.2 _(runtime — needs sidecar)_ Confirm a sidecar built from that branch with `agent.thinking.enabled: true` is reachable at `127.0.0.1:7821` for verification
...
- [ ] 2.3 _(runtime — needs sidecar)_ Verify the relay: with thinking enabled, send a prompt and confirm the three `agent://reasoning_*` events arrive in order
...
- [ ] 6.1 _(runtime — needs sidecar)_ End-to-end in `npm run tauri:dev` against a thinking-enabled sidecar: reasoning section appears, streams, settles with a duration, collapses; redacted block shows a marker; auto-expand preference behaves both on and off
- [ ] 6.2 _(runtime — needs sidecar)_ Confirm graceful degradation: against a sidecar without thinking (or disabled), no reasoning section appears and chat is unaffected
